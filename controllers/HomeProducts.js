    ...
    async catProducts(req, res) {
        ...
        if(page) {
            try {
                ...
                const response = await ProductModel.find({...options}).where("stock").gt(0).skip(skip).limit(perPage).populate("reviews").sort({updatedAt: -1});
                return res.status(200).json({products: response, perPage, count});
            } catch (error) {
                return res.status(500).json({error: error.message});
            }
        } else {
            const response = await ProductModel.find({...options}).where("stock").gt(0).limit(4).populate("reviews").sort({updatedAt: -1});
