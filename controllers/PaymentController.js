...
class PaymentController {
    ...
    async checkOutSession(request, response){
        ...
        switch (event.type) {
          ...
          case 'checkout.session.completed':
            ...
            let customer = await stripe.customers.retrieve(checkoutSessionCompleted.customer);
    ...
    async paymentVerification(req, res) {
      ...
      try {
        ...
        return res.status(200).json({msg: 'Payment has been verifies successfully!!', status: session.payment_status});
      } catch (error) {
